...
export default function Dashboard() {
  ...
  <button
    onClick={() => {
      // Show the recommendation modal first (old behavior)
      setPatchQuery(v.cve || v.id || v.name || "");
      setPatchModalOpen(true);
      // keepMessage, job run happens from inside PatchRecommendation component
    }}
  >
    Patch Now
  </button>
  ...
  // pass the sandbox runner so the modal can trigger a sandbox job
  onRunPatch={async (scriptPath, workdir = "/sandbox/data/testapp", jobName = "patch-job") => {
    setMessage("⏳ Submitting patch job to sandbox...");
    const res = await handleRunPatch(scriptPath, workdir, jobName);
    // optionally close modal on success:
    if (res && res.ok) {
      // keep modal open if you want to show output; otherwise close:
      // setPatchModalOpen(false);
      setMessage("✅ Patch job submitted. Check Sandbox Jobs for results.");
    } else {
      setMessage(`❌ Patch job failed: ${res?.error || (res && res.data && (res.data.error || res.data.detail)) || "unknown"}`);
    }
    return res;
  }}
/>
