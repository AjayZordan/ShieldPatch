...
export default function PredictRisk() {
  ...
  payload.description = trimmed;
      ...
      let predictedValue = data?.predictions?.[0]?.predicted;
...
if (isNaN(predictedValue)) {
  predictedValue = 0;
      ...
      // simple CVSS inference for UI
let inferredCvss = null;
const desc = query.toLowerCase();

if (desc.includes("critical")) inferredCvss = 9.5;
else if (desc.includes("overflow")) inferredCvss = 8.5;
else if (desc.includes("sql")) inferredCvss = 7.5;
else if (desc.includes("xss")) inferredCvss = 6.5;
else inferredCvss = 4.0;

if (cvss === "" && inferredCvss) {
  ...
  setCvssInferred(true);
      ...
      const newResult = {
  predicted_score: isNaN(predictedValue) ? 0 : Math.min(predictedValue * 10, 100),// convert to %
  logged: true, // assume saved (or change later if needed)
  id: null,
  raw: data,
};
